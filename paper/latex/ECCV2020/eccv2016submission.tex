...
\begin{abstract}
Deep convolutional neural networks exhibit exceptional performance in predicting depth from stereo images. However, in most cases, they suffer from two fundamental problems (a) they consume extraordinary computational power (clusters of GPUs) even for predicting a single depth image (b) their memory and computational demands are set from the training phase, so they cannot be altered at test time according to the capacities of each application
For confronting these problems, we propose a scalable CNN architecture (MSNet) that adjusts to the specific requirements on-demand; can reduce its computational cost with some accuracy reduction and vice-versa. This preference for accuracy or efficiency is determined at test time, without any need for retraining. For achieving this scalability,  we adopted the basic ideas of scale-space theory, and we adjusted them into the MSNet architecture.
