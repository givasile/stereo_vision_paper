...
The experimental evaluation is presented in three steps; (a) we compare MSNet with the State-of-the-art models (b) we compare MSNet to similar backbones CNN architectures but with important differences, creating an internal benchmark and (c) we experiment on how MSNet behaves under different set of processing scales. All experiments are based on the synthetic SceneFlow dataset \cite{MIFDB16}.
...
\begin{figure}[htb!]
    \centering
    \includegraphics[width=0.49\textwidth]{figures/msnet_scales_evaluation.pdf}
