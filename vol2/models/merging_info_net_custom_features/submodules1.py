...
class descriptors_extraction(torch.nn.Module):
    def __init__(self, ch, num_of_residuals_on_f2,
        ...
        # basic_module
        self.module_2d = module_2d(ch, num_of_residuals_on_f2,
                                   num_of_residuals_on_f3)
    ...
    def forward(self, im, scales, prediction_from_scales, device):
        ...
        for i in range(len(scales)):
            ...
            tmp1, tmp2 = self.module_2d(tmp, sc[1:], extract, device)
...
def softargmax(vol, new_dimension, device):
    vol = torch.nn.functional.interpolate(
        vol, new_dimension, mode='trilinear', align_corners=True)
    ...
    # prepare coeffs
    tmp = torch.linspace(
        0, vol.shape[1]-1, steps=vol.shape[1], requires_grad=False)
