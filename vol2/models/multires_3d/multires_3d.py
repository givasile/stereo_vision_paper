...
class cnn_3D(torch.nn.Module):
    def __init__(self):
        ...
        # 0
        self.first_layer = torch.nn.Sequential(basic_layer_3d(64))
                                               
        # down -> 1
        self.conv_stride_1 = torch.nn.Sequential(convbn_3d(64, 64, 3, 2, 1, 1),
                                                 torch.nn.ReLU(
                                                     inplace=True),
                                                 convbn_3d(
                                                     64, 64, 3, 1, 1, 1),
        ...
        # down -> 2        
        self.conv_stride_2 = torch.nn.Sequential(convbn_3d(64, 64, 3, 2, 1, 1),
                                                 torch.nn.ReLU(
                                                     inplace=True),
                                                 convbn_3d(
                                                     64, 64, 3, 1, 1, 1),
        ...
        # up -> 1
        self.conv_up_1 = torch.nn.Sequential(torch.nn.ConvTranspose3d(
            64, 64, kernel_size=3, padding=1, output_padding=1, stride=2, bias=False),
                                             torch.nn.BatchNorm3d(64),
                                             torch.nn.ReLU(inplace=False),
                                             torch.nn.Conv3d(64, 64, 3, 1, 1, 1, bias=False),
                                             torch.nn.BatchNorm3d(64),
                                             torch.nn.ReLU(inplace=False))
        ...
        # to regression
        self.to_regression = torch.nn.Sequential(basic_layer_3d(64),
                                                 torch.nn.Conv3d(64, 1, 3, 1, 1, 1, bias=False))
