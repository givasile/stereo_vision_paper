...
if action is 'from_scratch':
    ...
    if get_standart_dataset:
        ...
        from_datasets = [{'kitti_2012': [0, 0, 0]},
                         ...
                         {'freiburg_flying': [8, 8, 12]}]
        dataset = merged.dataset(from_datasets)
    with open(save_to + '/merged_dataset.pickle', 'wb') as fm:
        pickle.dump(dataset, fm)

    # initialize stats
    stats = {'train': {'loss': [], 'mae': [], 'std_ae': [], 'pcg': []},
             'val_crop': {'mae': [], 'std_ae': [], 'pcg': []},
             'val_full': {'mae': [], 'std_ae': [], 'pcg': []},
             'test_crop': {'mae': [], 'std_ae': [], 'pcg': []},
             'test_full': {'mae': [], 'std_ae': [], 'pcg': []},
             'general': {'cur_epoch': 1,
                         'cur_step': 1,
...
for ep in range(init_epoch, init_epoch + train_for_epochs):
    ...
    # TRAINING PART
    if train_on_crop:
        net.training_epoch(dataset, 2, stats, model_instance, optimizer, maxD, device)
    net.save_checkpoint(model_instance, optimizer, stats, save_to)
    torch.cuda.empty_cache()
    
    # VAL_CROP
    if val_on_val_crop:
        net.validate('val', 'crop', dataset, 4, stats, model_instance, device, maxD)
    net.save_checkpoint(model_instance, optimizer, stats, save_to)
    torch.cuda.empty_cache()
    
    # VAL_FULL
    if val_on_val_full:
        net.validate('val', 'full', dataset, 2, stats, model_instance, device, maxD)
    net.save_checkpoint(model_instance, optimizer, stats, save_to)
    torch.cuda.empty_cache()
    
    # TEST_CROP
    if val_on_test_crop:
        net.validate('test', 'crop', dataset, 16, stats, model_instance, device, maxD)
    net.save_checkpoint(model_instance, optimizer, stats, save_to)
    torch.cuda.empty_cache()
    
    # TEST_FULL
    if val_on_test_full:
        net.validate('test', 'full', dataset, 4, stats, model_instance, device, maxD)
    net.save_checkpoint(model_instance, optimizer, stats, save_to)
    torch.cuda.empty_cache()
