...
class descriptor(torch.nn.Module):
    def __init__(self):
        ...
        # first seq
        self.layer_1_1 = torch.nn.Sequential(convbn(16, 32, 3, 1, 1, 1),
                                             torch.nn.ReLU(inplace=True),
                                             convbn(32, 32, 3, 1, 1, 1),
                                             torch.nn.ReLU(inplace=True),
                                             torch.nn.Conv2d(32, 16, 3, 1, 1, 1))
        self.layer_1_2 = torch.nn.Sequential(downsample(16, 16),
                                             basic_layer(16),
                                             basic_layer(16))

        # second seq
        self.layer_2_1 = torch.nn.Sequential(convbn(16, 32, 3, 1, 1, 1),
                                             torch.nn.ReLU(inplace=True),
                                             convbn(32, 32, 3, 1, 1, 1),
                                             torch.nn.ReLU(inplace=True),
                                             torch.nn.Conv2d(32, 16, 3, 1, 1, 1))
        self.layer_2_2 = torch.nn.Sequential(downsample(16, 32),
                                             basic_layer(32),
                                             basic_layer(32))

        # third seq
        self.layer_3_1 = torch.nn.Sequential(convbn(32, 64, 3, 1, 1, 1),
                                             torch.nn.ReLU(inplace=True),
                                             convbn(64, 64, 3, 1, 1, 1),
                                             torch.nn.ReLU(inplace=True),
                                             torch.nn.Conv2d(64, 16, 3, 1, 1, 1))
        self.layer_3_2 = torch.nn.Sequential(downsample(32, 64),
                                             basic_layer(64),
                                             basic_layer(64))
        # fourth seq 
        self.layer_4_1 = torch.nn.Sequential(convbn(64, 64, 3, 1, 1, 1),
                                             torch.nn.ReLU(inplace=True),
                                             convbn(64, 64, 3, 1, 1, 1),
                                             torch.nn.ReLU(inplace=True),
                                             torch.nn.Conv2d(64, 16, 3, 1, 1, 1))
        self.layer_4_2 = torch.nn.Sequential(downsample(64, 64),
                                             basic_layer(64),
                                             basic_layer(64))

        # fifth seq
        self.layer_5_1 = torch.nn.Sequential(basic_layer(64))
...
class cnn_3D(torch.nn.Module):
    def __init__(self):
        ...
        # res5
        self.res5_1 = torch.nn.Sequential(conv_3d(128, 64, 3, 1, 1, 1),
                                          torch.nn.ReLU(inplace=False),
                                          basic_layer_3d(64),
