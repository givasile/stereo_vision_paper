...
if tmp is not None:
    mae, pcg, pred, imL_d, imR_d, cost_a, cost_b = tmp
...
# error
threshold = 3
err_im = evaluate.image_absolute_error(pred[0], dispL, maskL)
over_thres_im = evaluate.image_percentage_over_limit(pred[0], dispL, maskL, threshold)
...
visualize.imshow_1ch(pred[0][0].cpu(), 'prediction_1', [
                     dispL.min().item(), dispL.max().item()])
visualize.imshow_1ch(pred[1][0].cpu(), 'prediction_2', [
                     dispL.min().item(), dispL.max().item()])
visualize.imshow_1ch(pred[2][0].cpu(), 'prediction_3', [
                     dispL.min().item(), dispL.max().item()])
visualize.imshow_1ch(pred[1][0].cpu(), 'prediction_4', [
                     dispL.min().item(), dispL.max().item()])
visualize.imshow_1ch(pred[2][0].cpu(), 'prediction_5', [
                     dispL.min().item(), dispL.max().item()])
